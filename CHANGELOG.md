...
### Fixed
- **Grid viewport scaling** - 8x8 grids now fit on screen properly
  - Added SizedBox constraint to limit grid to available screen space
  - Grid uses smaller of width/height to ensure it fits
  - No more cut-off grids on any screen size
  - All grid sizes (4x4 through 8x8) now scale to fit viewport

## [1.5.1] - 2026-08-03

### Fixed
- **Aspect ratio scaling** - font sizing now uses the smaller of width/height dimensions
  - Ensures proper scaling on both wide and tall screens
  - Text fits correctly regardless of screen aspect ratio (portrait/landscape)
  - Fixes issue where wide screens only scaled based on width
...
### Added
- **Word count in results screen** - displays number of words found alongside Score and Time
  - Layout: Score | Words | Time
  - Shows total words found during the game
